...
const Wire = ({
    ...
    const acquirePacket = ({payload, meta, match}) => {
        if (match && match.idx && match.tag) {
            ...
            if (matched) {
                ...
                return acquirePacket({payload, meta});
            }
        } else {
            const found = packets.find(meta);
            if (!found) {
                return {
                    packet: packets.add({
                        packet: {
        ...
        setTimeout(async() => {
            const methodCallResult = await callMethod({
                ...
                meta: {
                    ...
                    trace: acquired.packet.trace,
                    direction: addDirection(acquired.packet.direction),
                    ...meta
                },
                match,
                ctx: {
