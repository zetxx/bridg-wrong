...
class Brid {
    ...
    async findMethod({method, channel, direction = 'out'} = {}) {
        this.log('debug', {in: 'base.findMethod', args: {method, channel, direction}});
    ...
    registerMethod({method, channel, fn, meta}) {
        this.log('debug', {in: 'base.registerMethod', args: {method, channel, meta}});
        !this.nodeMethods[channel] && (this.nodeMethods[channel] = {});
        this.nodeMethods[channel][method] = async(ctx, ...args) => {
            this.log('debug', {in: 'base.registerMethod:registeredMethodExecution', args: {method, channel, meta, ctx, methodArgs: {ctx, ...args}}});
    ...
    isApiRequest(apiRequestId) {
        this.log('debug', {in: 'base.isApiRequest', args: {apiRequestId}});
        return this.apiRequestsPool.findIndex(({meta} = {}) => meta.apiRequestId === apiRequestId);
    }

    isApiRequestByMatchKey({result}) {
        this.log('debug', {in: 'base.isApiRequestByMatchKey', args: {result}});
    ...
    getInternalCommunicationContext(meta, extraContext = {}) {
        ...
        return {
            ...
            request: async(destination, message, metaParams = {}) => {
                this.log('debug', {in: 'base.getInternalCommunicationContext.request', args: {destination, message, metaParams, meta, extraContext}});
            ...
            notification: async(destination, message, metaParams = {}) => {
                this.log('debug', {in: 'base.getInternalCommunicationContext.notification', args: {destination, message, metaParams, meta, extraContext}});
                this.remoteApiRequest({destination, message, meta: Object.assign({}, meta, metaParams, {isNotification: 1})})
                    .catch((error) => this.log('error', {
                        ...
                        text: 'error should be handled correctly',
                        error,
    ...
    async remoteApiRequest({destination, message, meta}) {
        this.log('debug', {in: 'base.remoteApiRequest', args: {destination, message, meta}});
        throw errors.notImplemented();
    }

    // when someone hits api method
    apiRequestReceived({message, meta}) {
        this.log('debug', {in: 'base.apiRequestReceived', args: {message, meta}});
        return new Promise(async(resolve, reject) => {
            ...
            try {
                ...
                this.log('debug', {in: 'base.apiRequestReceived', text: 'api "in" method found', args: {message, meta, m, pack}});
                if (meta.isNotification) { // respond if notification, dont wait for anything to be executed
                    this.log('debug', {in: 'base.apiRequestReceived', text: 'api "in" method found, but notification', args: {message, meta, m, pack}});
                ...
                if (apiMethodFnResult) {
                    this.log('info', {in: 'base.apiRequestReceived > send to external', args: {message, meta: m, pack, apiMethodFnResult}});
                    ...
                    if (!meta.isNotification && m && (m.apiRequestId || m.apiRequestId >= 0) && this.isApiRequest(m.apiRequestId) >= 0) {
                        this.log('info', {in: 'base.apiRequestReceived > timeout started', args: {message, meta: m, pack, apiMethodFnResult, extOutResP}});
                        ...
                        var timeoutId = setTimeout(() => {
                            this.log('info', {in: 'base.apiRequestReceived > method timeout', args: {message, meta: m, pack, apiMethodFnResult, extOutResP}});
                ...
                this.log('info', {in: 'base.apiRequestReceived > skip external, result from api fn call is false', args: {message, meta: m, pack, apiMethodFnResult}});
                return (!meta.isNotification && this.apiResponseReceived({result: apiMethodFnResult, meta: m}));
            } catch (e) {
                this.log('error', {in: 'base.apiRequestReceived', error: e, args: {message, meta: m, pack}});
    ...
    async apiResponseReceived({result, error, meta: {apiRequestId = -1}} = {}) {
        ...
        this.log('debug', {in: 'base.apiResponseReceived', args: {result, error, apiRequestId}});
        if (this.isApiRequest(apiRequestId) >= 0) { // request exists
            ...
            if (meta.isNotification) { // notification case
                this.log('debug', {in: 'base.apiResponseReceived', text: 'api "in" method found, but notification, returning notification: 1', args: {result, error, apiRequestId}});
            ...
            try {
                ...
                this.log('debug', {in: 'base.apiResponseReceived', text: 'resolve request', args: {result, error, apiRequestId}});
    ...
    async externalIn({result, error, meta = {}} = {}) {
        this.log('debug', {in: 'base.externalIn', args: {result, error, meta}});
        ...
        if (apiRequestId) { // try to find api request in order to apply response
            if (this.isApiRequest(apiRequestId) >= 0) { // this message is response of api request
                try {
                    ...
                    this.log('debug', {in: 'base.externalIn', text: 'external request is response of api request', args: {result, error, meta, fnExtResult}});
                    // call api response
            ...
            if (apiRequestIdByMatchKey > 0) {
                this.log('debug', {in: 'base.externalIn', text: 'external request is response of api request, matched by "apiRequestIdByMatchKey"', args: {result, error, meta}});
                ...
                this.log('debug', {in: 'base.externalIn', text: 'external request is NOT response of api request', args: {result, error, meta}});
                try {
                    let fnExt = await this.findExternalMethod({method});
                    ...
                    if (transformedMsg) { // respond to external if result from transform function is not false.
                        this.log('debug', {in: 'base.externalIn', text: 'external request is NOT response of api request > send response to external', args: {result, error, meta}});
                        return this.externalOut({result: transformedMsg, meta});
                    }
                    this.log('debug', {in: 'base.externalIn', text: 'external request is NOT response of api request > dont send response to external because user transofrm function returned false', args: {result, error, meta, transformedMsg}});
                    return {...message, meta: Object.assign({deadIn: 1}, meta)};
                } catch (e) {
