...
class Brid {
    ...
    registerMethod({method, channel, fn}) {
        this.log('info', {in: 'method:registerMethod'});
    ...
    externalIn({message, meta = {}} = {}) {
        ...
        if (apiRequestId) { // try to find api request in order to apply response
            if (this.isApiRequest(apiRequestId) >= 0) { // this message is response of api request
                ...
                return this.findExternalMethod({method}).then((fn) => fn(this.getInternalCommunicationContext(Object.assign({direction: 'in'}, meta)), message))
                    .catch((error) => {
    ...
    externalOut(...args) {
        this.log('info', {in: 'method:externalOut'});
